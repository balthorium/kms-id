...
5. Client A generates a new GMBC by creating a genesis block using the reserved GMBC URI.  The genesis block references the resource URI obtained in (4) and includes two group membership "add" operations (one for Client A and one for Client B).  The URI of the KMS server is used in the curator field.
